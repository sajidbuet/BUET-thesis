...
\item \Cref{sec:cite} in \Cref{ch:citations} shows some examples of
  citations.
  \item \Cref{fig:sinewave} is a vector figure drawn with Tikz.
\end{itemize}
...
You can also use TikZ package to directly generate vector drawings. For example see \Cref{fig:sinewave}.
...
% Define the function parameters
  ...
  % Draw axes
  \draw[->] (-2*pi,0) -- (2*pi,0) node[below] {$x$};
  \draw[->] (0,-1.5) -- (0,1.5) node[left] {$y$};
  
  % Draw sine wave
  \draw[domain=-2*pi:2*pi,smooth,variable=\x,blue] plot ({\x},{\A*sin(\f*\x r + \phi)});
  
  % Label x-axis in terms of pi
  \foreach \x/\label in {-2*pi/-2\pi,-pi/-\pi,pi/\pi,2*pi/2\pi}
    \draw (\x,-0.1) -- (\x,0.1) node[above] {$\label$};
  
  % Label y-axis
  \foreach \y/\label in {-1/-\A,1/\A}
    \draw (-0.1,\y) -- (0.1,\y) node[right] {$\label$};
\end{tikzpicture}
  \caption{Sine Wave.}
  \label{fig:sinewave}
\end{figure}
